...
const getCommonGames = (a, b) => {
  ...
  const commonGames = smallerSet.filter((appID) => largerSet.includes(appID)); 
  return commonGames;
};
...
const getSharedGames = async (steamIDs) => {
  ...
  } else log.warn('You must input more than one steamID');
  ...
  return filteredGames.filter((game) => {
    ...
    while (platformIndex < filters.platforms.length) {
      filters.platforms.forEach((platform) => {
        if (game.platforms[platform] === false) includesAllPlatforms = false;
      });
      platformIndex++; 
    }
